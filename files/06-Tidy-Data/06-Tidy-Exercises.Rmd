...
Reshape the 5th through 60th columns of `who` into a pair of name:value columns named `codes` and `n`. (Hint: make the dataset longer.)
...
Check the documentation for `pivot_longer` and try it without using `separate`.

```{r}
who %>% 
  pivot_longer(
  cols = 5:60,
  names_to = _______________________________,
  names_pattern = "new_?(.*)_(.)(.*)",
  values_to = _______________________________
  ) %>% 
  select(-iso2, iso3)
```
